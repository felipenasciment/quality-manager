package managedBean;

public class PathServices {

	// Services Gestor
	public static final String CADASTRAR_INSTITUICAO_FINANCIADORA = "cadastrar/instituicaofinanciadora";
	public static final String CONSULTAR_INSTITUICOES_FINANCIADORAS = "consultar/instituicoesfinanciadoras";
	public static final String CADASTRAR_PROGRAMA_INSTITUCIONAL = "cadastrar/programainstitucional";
	public static final String CONSULTAR_PROGRAMAS_INSTITUCIONAIS = "consultar/programasinstitucionais";
	public static final String CADASTRAR_EDITAL = "cadastrar/edital";
	public static final String CONSULTAR_EDITAIS = "consultar/editais";
	public static final String CONSULTAR_INSTITUICOES_BANCARIAS = "consultar/instituicoesbancarias";
	public static final String CONSULTAR_ORIENTADORES = "consultar/orientadores";
	public static final String CADASTRAR_CURSO = "cadastrar/curso";
	public static final String CONSULTAR_CURSOS = "consultar/cursos";
	
	
	// Services Docente
	public static final String CADASTRAR_PROJETO = "cadastrar/projeto";
	public static final String CADASTRAR_ORIENTADOR = "cadastrar/orientador";
	public static final String CONSULTAR_PROJETOS = "consultar/projetos";
	
	//Services Coordenador
	public static final String CADASTRAR_DISCENTE = "cadastrar/discente";
	public static final String CONSULTAR_DISCENTES = "consultar/discentes";
	
	//Services Usuário
	public static final String FAZER_LOGIN = "consultar/fazerlogin";
	
	
}
